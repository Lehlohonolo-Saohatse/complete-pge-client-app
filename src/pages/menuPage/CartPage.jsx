...
const CartPage = () => {
    ...
    return (
      <div className="max-w-screen-2xl container mx-auto xl:px-24 px-4">
        <div className="h-screen flex flex-col items-center justify-center">
          ...
          <Link to="/login" className="btn bg-green text-white">
            Login
          </Link>
        </div>
      </div>
  ...
  const handleIncrease = async (item) => {
    try {
      const response = await fetch(`https://complete-pge-server.onrender.com/carts/${item._id}`, {
        method: "PUT",
        headers: {
    ...
    if (item.quantity > 1) {
      try {
        const response = await fetch(
          `https://complete-pge-server.onrender.com/carts/${item._id}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
  ...
  // Calculate the cart subtotal safely
  ...
  const handleDelete = (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`https://complete-pge-server.onrender.com/carts/${item._id}`).then(response => {
          if (response) {
            refetch();
    ...
    return (
      <div className="max-w-screen-2xl container mx-auto xl:px-24 px-4">
        <div className="h-screen flex flex-col items-center justify-center">
          ...
          <Link to="/menu" className="btn bg-green text-white mt-4">
            Back to Menu
          </Link>
        </div>
      </div>
        ...
        <div>
                ...
                <tbody>
                  {cart.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td>R{calculateTotalPrice(item).toFixed(2)}</td>
                      <td>
                        <button
          ...
          <div className="flex flex-col md:flex-row justify-between items-start my-12 gap-8">
            ...
            <div className="md:w-1/2 space-y-3">
              ...
              <p>
                Total Price:{" "}
                <span id="total-price">R{orderTotal.toFixed(2)}</span>
              </p>
